    ...
    <div class="h-[500px] grid grid-cols-3">
      <div class="bg-gray-200 col-span-1 justify-center grid grid-rows-2 rounded-l-md">
        <div class="flex items-center justify-center">
          <img
            :src="toRaw(userData.matchedUser?.profile?.userAvatar)"
            alt="Your avatar"
        ...
        <div class="flex items-start flex-col">
          ...
          <p class="text-black text-2xl text-center">Total active days: {{ toRaw(userData.matchedUser?.userCalendar?.totalActiveDays) }}</p>
        </div>
      </div>
      <div class="col-span-2 rounded-r-md grid grid-rows-5 h-full w-full m-0">
        <div class="bg-green-100 grid grid-cols-4 row-span-3 h-full w-full m-0 rounded-tr-lg">
          <div class="flex flex-col justify-start items-center">
            <p class="text-black text-center text-2xl font-bold mt-5">{{ toRaw(problemData.allQuestionsCount?.[1]?.difficulty) }}</p>
            <div
              class="mt-5"
              role="progress-circle"
              :style="{
                '--size': '180px',
                '--value': toRaw(userData.matchedUser?.submitStats?.totalSubmissionNum?.[1]?.count),
                '--total': toRaw(problemData.allQuestionsCount?.[1]?.count),
                '--primary': 'green',
          ...
          <div class="flex flex-col justify-start items-center">
            <p class="text-black text-center text-2xl font-bold mt-5">{{ toRaw(problemData.allQuestionsCount?.[2]?.difficulty) }}</p>
            <div
              class="mt-5"
              role="progress-circle"
              :style="{
                '--size': '180px',
                '--value': toRaw(userData.matchedUser?.submitStats?.totalSubmissionNum?.[2]?.count),
                '--total': toRaw(problemData.allQuestionsCount?.[2]?.count),
                '--primary': 'orange',
          ...
          <div class="flex flex-col justify-start items-center">
            <p class="text-black text-center text-2xl font-bold mt-5">{{ toRaw(problemData.allQuestionsCount?.[3]?.difficulty) }}</p>
            <div
              class="mt-5"
              role="progress-circle"
              :style="{
                '--size': '180px',
                '--value': toRaw(userData.matchedUser?.submitStats?.totalSubmissionNum?.[3]?.count),
                '--total': toRaw(problemData.allQuestionsCount?.[3]?.count),
                '--primary': 'red',
                '--secondary': 'rgb(240, 112, 112)'
              }"
            ></div>
          </div>
          <div v-if="userData.matchedUser?.languageProblemCount?.length !== 0" class="flex flex-col justify-center ml-5">
            <p class="text-black text-2xl font-bold">Top Languages</p>
            <p v-for="i in NUM_LANGUAGE_DISPLAY" :key="userData.matchedUser.languageProblemCount?.[i-1]?.languageName || i" class="text-black text-2xl">
              {{ toRaw(userData.matchedUser.languageProblemCount?.[i - 1]?.languageName) }}
              :
              {{ toRaw(userData.matchedUser.languageProblemCount?.[i - 1]?.problemsSolved) }}
            </p>
          </div>
        </div>
        <div class="bg-gray-100 row-span-2 w-full h-full m-0 place-content-center">
          <Calendar
            :submissionDates="toRaw(userData.matchedUser?.userCalendar?.submissionCalendar)"
...
import { reactive, defineProps, watch, onMounted, toRaw } from 'vue';
...
async function fetchAllProblemCounts() {
  try {
    const response = await $fetch(`/api/problem`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token.value}`, // Access token.value here
      },
    });
    ...
    reportError(error, { section : `users/${username.value}/problem_data`});
  }
}
...
async function fetchUserProfile() {
  ...
  try {
    ...
    if (response.data) {
      ...
      activeYears.value = toRaw(userData.matchedUser?.userCalendar?.activeYears || []);
    }

    isLoadingCalendar.value = false;
  } catch (error: any ) {
    reportError(error, { section : `users/${username.value}/profile_data`});
  }
}
</script>
